...
twice, there is only one copy. Changing one view will change the other. Now we
can change the buffer being displayed in one window.
...
to that file. But many buffers are not and, instead, contain other
information.
